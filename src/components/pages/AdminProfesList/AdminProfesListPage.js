...
import EditProfeModal from "../../EditProfesorModal";
...
const AdminProfesListPage = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      const response = await getFetch(`${host}planes/profesores/busqueda/`);
  ...
  const handleProfeUpdate = (updatedProfe) => {
    const putData = async () => {
      try {
        await putFetch(`${host}planes/profesores/`, updatedProfe);
  ...
  const handleConfirmDelete = async () => {
    try {
      await deleteFetch(`${host}planes/profesores/`, { id: deleteId });
